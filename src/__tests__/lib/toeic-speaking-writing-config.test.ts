...
import { EXAM_CONFIG } from "@/lib/toeic-speaking-writing/config";

describe("TOEIC Speaking & Writing Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM008");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("toeic-speaking-writing");
  });
